...
import { validateRequestBody } from './validationMiddleware';
...
apiRouter.get('/health', (_req: Request, res: Response) => {
  ...
  res.json({
    ...
    envChecked: Boolean(process.env.GEMINI_API_KEY || process.env.VITE_GEMINI_API_KEY || process.env.GOOGLE_API_KEY),
    timestamp: new Date().toISOString(),
  });
});

// 2. GitHub Manifest & Category stats
apiRouter.get('/github/manifest', async (_req: Request, res: Response) => {
  try {
...
// 3. Search & Pagination for 15,000+ Prompts
apiRouter.get('/github/prompts', async (req: Request, res: Response) => {
  try {
...
// 4. API: Generate Image with Imagen 3 / Gemini
apiRouter.post('/generate-image', generateImageValidator, async (req: Request, res: Response) => {
  res.setHeader('Content-Type', 'application/json');
...
// 5. API: Enhance Prompt / Content Remix with Gemini
apiRouter.post('/enhance-prompt', enhancePromptValidator, async (req: Request, res: Response) => {
  res.setHeader('Content-Type', 'application/json');
  try {
...
// 6. API: Analyze Image / Reverse Prompt Engineering
